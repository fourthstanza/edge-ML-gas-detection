...
\begin{itemize}
    ...
    \item A dataset to train your model on, or a trained TensorFlow Lite model
\end{itemize}
...
\paragraph{Why are we using Tensorflow/tflite, and not more modern frameworks like PyTorch or JAX?} Since we're deploying on the edge, we need a workflow with aggressive optimization methods. TFLite is currently the only largely adopted deep edge nn model framework. It natively uses TensorFlow models and supports direct conversion from Keras. TF, ONNX, PyTorch etc target devices like phones and low-powered linux computers on the edge. 
