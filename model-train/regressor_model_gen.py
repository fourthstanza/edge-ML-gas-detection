...
class DataSplit:
    ...
    def __init__(self, X: np.ndarray, y:np.ndarray, win_length: int, split:  float | tuple[float, float] = 0.8, time: np.ndarray | None = None, seed: int | None = None):
    ...
    def split(self, split: float, seed: int | None = None):
        ...
        self.tr_ind = random.sample(datarange, k = int(len(datarange) * split))
...
def create_data_split(data: np.ndarray, split: float | tuple[float, float], seed: int | None = None) -> DataSplit:
    ...
    splt = DataSplit(X, y, winlength, split, time = t, seed = seed)
...
def main(): 
    ...
    data_split = create_data_split(data, split = 0.8, seed = 42)
